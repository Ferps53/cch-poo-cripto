    ...
    private void adicionarNovaCriptoMoeda(OpcoesCripto opcoesCripto) {
        ...
        for (final var nomeNaTabela : tabela.getNomeTickers()) {
            if (nomeNaTabela.equals(opcoesCripto.getAbreviacao())) {
                final var painelErro = new PainelErro("A moeda " + opcoesCripto.getAbreviacao() + " já foi cadastrada!", painel);
                painel.add(painelErro);
                // revalidate() e repaint(), fazem a dialog ser recalculado e pintar novamente os componentes.
        ...
        // Inicia a chamada do endpoint em uma nova thread. Dessa forma usa de paralelismo para não travar a UI!
        Thread.ofVirtual().start(() -> {
            try {
                final String body = CLIENTE.buscaDados("https://www.mercadobitcoin.net/api/" + opcoesCripto.getAbreviacao() + "/ticker");
